 ...
 * @Last Modified time: 2020-08-04 21:50:14
 */
...
const installApp = async (DOWNLOAD_DIR, projectInfo) => {
  ...
  if (!_.isEmpty(versionInfo) && versionInfo.length > 0) {
    ...
    const start_project = projectInfo.env ? `npm run dev` : `pm2 start server.js --name ${projectInfo.name}`;
    ...
    shell.exec(`npm install pm2 -g --registry=${NPM_REGISTRY}`);
